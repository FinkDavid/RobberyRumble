...
{
    std::vector<tson::Object> dynamicColliderObjects{};
    ...
    for (auto& layer : map->getLayers())
    {
        ...
        {
            if (object.getType() == "DynamicCollider")
            {
                dynamicColliderObjects.push_back(object);
                continue;
            }
        }
    }
    ...
    for (auto& layer : map->getLayers())
    {
        ...
        for (auto& object : layer.getObjects())
        {
            ObjectFactory::processTsonObject(object, layer, resourcePath, spriteManager, dynamicColliderObjects);
        }
    }
